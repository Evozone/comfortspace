...
const ViewBlog = ({ mode }: { mode: string }) => {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const params = useParams<{ id: string }>();
    const blogId = params.id;
    const author = useSelector((state: { auth: AuthState }) => state.auth);
    ...
    const deleteBlog = async () => {
        ...
        try {
            ...
            console.log(error);
            dispatch(notifyAction(true, 'error', 'Sorry :( but something went wrong!'));
        }
        dispatch(stopLoadingAction());
    };
